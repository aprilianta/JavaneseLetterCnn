...
DEFAULT_LABEL_FILE = os.path.join(SCRIPT_PATH,
                                  'labels/label_aksara_jawa-tanpa-suku.txt')
DEFAULT_TFRECORDS_DIR = os.path.join(SCRIPT_PATH, 'tfrecords-output-skenario-tanpa-suku')
DEFAULT_OUTPUT_DIR = os.path.join(SCRIPT_PATH, 'saved-model-skenario-tujuh')

MODEL_NAME = 'aksara_jawa_model_tanpa_suku'
IMAGE_WIDTH = 64
IMAGE_HEIGHT = 64

DEFAULT_NUM_TRAIN_STEPS = 31000
BATCH_SIZE = 100
...
def main(label_file, tfrecords_dir, model_output_dir, num_train_steps):
    ...
    """
    labels = io.open(label_file, 'r', encoding='utf-8').read().splitlines()  #buka file label dlm bentuk .txt
    num_classes = len(labels) #tentukan num_class dr file label
